...
class RDTSender:
    ...
    def __init__(self,
        ...
        self.port = port
        print_info(f"Sender Port: {self.port}", PrintType.DEBUG)
    ...
    def send_data(self, data: list[bytes]):
        print_info(f"{self.socket}, {self.listener}", PrintType.DEBUG)
        if not self.socket:
            print_info("Sender: Connection must be established before sending!", PrintType.ERROR)
        ...
        data_list = data.copy()  # Create a copy of the data to ensure original data is not overwritten
        starting_seq = self.seq_num  # Important for checking if a packet is the last one
        print_info(f"Sender: Starting Seq - {starting_seq} | Seq Base: {self.seq_base}", PrintType.DEBUG)
        ...
        try:
            ...
            while (len(data_list) > 0 or any(not ack for ack in self.received_acks.values())) and self.is_sending:
                ...
                # sending
                while self.seq_base <= self.seq_num < self.seq_base + self.window_size and len(data_list) > 0 \
                        and self.is_sending:
                    # Pop the next chunk and send the packet with the current seq num
                    chunk = data_list.pop(0)
                    self.send_packet(self.seq_num, chunk, self.seq_num == starting_seq + len(data) - 1)
                    print_info(f"Sender: Sent {self.seq_num}", PrintType.DEBUG)
                ...
                with self.lock:
                    ...
                    while self.seq_base in self.window and self.received_acks.get(self.seq_base, False) \
                        ...
                        self.seq_base += 1
                        print_info(f"Sender: {self.seq_base}", PrintType.DEBUG)
                ...
                if self.is_sending:
                    with self.lock:
                        ...
                        for seq in list(self.window.keys()):
                            ...
                            if ((seq in self.received_acks and not self.received_acks[seq]) and
                                    current_time - self.window[seq][1] > datetime.timedelta(seconds=self.timeout) and
                                    self.is_sending):
                                print_info(f"Sender: Retransmitting {seq}", PrintType.DEBUG)
            ...
            self.is_sending = False
            print_info(f"Sender: All data sent and acknowledged or connection terminated. | is_sending {self.is_sending}", PrintType.SUCCESS)


        except Exception as e:
            print_info(f"Sender Error: {str(e)}", PrintType.ERROR)
    ...
    def send_packet(self, seq_num: int, data: bytes, final_pkt=False):
        ...
        """
        print_info(f"Sender: Final Packet {final_pkt}", PrintType.DEBUG)
        payload = {
            "final": final_pkt,
            "seq": seq_num,
            "data": data,
        ...
        try:
            ...
            print_info(f"Sender: Sending {final_pkt}", PrintType.DEBUG)
            self.socket.sendall(pkt_len + final_payload)
        except OSError:
            print_info("Sender: End host socket was closed or an error occurred on the OS, terminating socket...",
                       PrintType.INFO)
            self.is_sending = False
            self.is_terminated = True
        except Exception as e:
            print_info(f"Failed to send packet: {e}", PrintType.ERROR)
        ...
        while self.is_sending or not self.is_terminated:
            try:
                ...
                if "ack" in pkt:
                    ...
                    print_info(f"Sender: Received ACK {pkt['ack']}", PrintType.DEBUG)

            except socket.timeout:  # Timeouts prevent the shared resources from being blocked by the thread
                continue
            except OSError as e:
                print_info(f"Sender: Socket closed or error: {str(e)}", PrintType.INFO)
                break
            except Exception as e:
                print_info(f"Sender: Exception in receive_acks: {str(e)}", PrintType.ERROR)
                break
    ...
    def terminate_connection(self):
        ...
        """
        print_info("Sender: Closing connection!", PrintType.INFO)
        self.is_sending = False
        ...
        if self.listener.is_alive() and threading.current_thread() is not self.listener:
            print_info("Sender: Joining receiving thread!", PrintType.INFO)
            self.listener.join()
            self.listener = None
